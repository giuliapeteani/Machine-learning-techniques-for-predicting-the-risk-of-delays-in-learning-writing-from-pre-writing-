...
clc
clear all
load('copy_tunnel.mat')
load('es_ios_t_e.mat')
load('sign_levels_r.mat')
t_e=5;
iOS=1;
        ...
        if ~isempty(azimuth_clean{child,1})
                ...
                if ~isempty(ampiezza_picchiAZ{child, 1})
                    if size(ampiezza_picchiAZ{child,1},1)==1
                        ...
                        ampiezza_secondo_picco_az(child,1)=NaN;
...
%% numero di errori di giro sommati su tutte le esecuzioni & numeo di 
%esecuzioni con almeno un errore
